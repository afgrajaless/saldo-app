...
export class CardResponseDto {
  ...
  @ApiProperty({ description: 'Día de cierre del período de facturación (1-31).', example: 15 })
  statementDay!: number;

  @ApiProperty({ description: 'Día de vencimiento del pago (1-31).', example: 25 })
  paymentDay!: number;

  @ApiProperty({
    description: 'Tasa de interés corriente E.A. del diferido rotativo (fracción decimal).',
    example: 0.28,
  })
  rotativoRateEa!: number;

  @ApiProperty({
    description: 'Pago mínimo como fracción decimal del saldo (ej. 0.05 = 5%).',
    example: 0.05,
  })
  ...
  @ApiProperty({
    description: 'Fecha límite de pago del ciclo actual en formato YYYY-MM-DD.',
    example: '2025-07-25',
  })
  ...
  @ApiProperty({
    description:
      'Indica si la tasa de interés rotativo supera la tasa de usura vigente para consumo ordinario.',
    example: false,
  })
  exceedsUsury!: boolean;

  @ApiProperty({ description: 'Fecha de creación de la tarjeta.' })
  createdAt!: Date;
}
